    ...
    <div class="container">
        ...
        <div class="header">
            ...
            <div class="header-right">
                <div class="doc-title">{{ strtoupper($invoice->type) }} INVOICE</div>
        ...
        <div class="info-section">
            ...
            <div class="info-col">
                ...
                @if($invoice->client->city)<div class="muted">{{ $invoice->client->city }}</div>@endif
        ...
        <div class="meta-bar">
            ...
            <div class="meta-item">
                ...
                <div class="meta-value">{{ $invoice->project_event ?? '-' }}</div>
            </div>
        </div>
        ...
        <div class="totals">
            <div class="totals-row">
                ...
                <span class="totals-value">Rs. {{ number_format($invoice->sub_total, 2) }}</span>
            </div>
            @if($invoice->discount_percent > 0)
            <div class="totals-row">
                <span class="totals-label">DISCOUNT ({{ $invoice->discount_percent }}%)</span>
                <span class="totals-value red">- Rs. {{ number_format($invoice->sub_total * ($invoice->discount_percent / 100), 2) }}</span>
            </div>
            @endif
            ...
            @endphp
            <div class="totals-row">
                <span class="totals-label">VAT ({{ $invoice->vat_percent }}%)</span>
                <span class="totals-value">Rs. {{ number_format($vatAmount, 2) }}</span>
            </div>
            @endif
            <div class="totals-row total-final">
                <span class="totals-label">TOTAL</span>
                <span class="totals-value">Rs. {{ number_format($invoice->total_amount, 2) }}</span>
            </div>
            @if($invoice->paid_amount > 0)
            <div class="totals-row">
                <span class="totals-label green">PAID</span>
                <span class="totals-value green">Rs. {{ number_format($invoice->paid_amount, 2) }}</span>
            </div>
            <div class="totals-row">
                <span class="totals-label orange bold">BALANCE DUE</span>
                <span class="totals-value orange bold">Rs. {{ number_format($invoice->balance_due, 2) }}</span>
            </div>
            @endif
        </div>
        ...
        @if($bankName)
        <div class="terms">
            <div class="terms-title">Bank Details for Payment</div>
            <div class="muted">{{ $bankName }} @if($bankBranch) — {{ $bankBranch }} @endif</div>
            @if($bankAccount)<div class="muted">Account: {{ $bankAccount }}</div>@endif
        </div>
        @endif
