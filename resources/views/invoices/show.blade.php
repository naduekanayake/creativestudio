...
<div class="no-print flex items-center justify-between mb-6">
   ...
   <div class="flex items-center gap-2">
        @php
            ...
            $waPhone = preg_replace('/[^0-9]/', '', $invoice->client->phone ?? '');
            if (substr($waPhone, 0, 1) === '0') { $waPhone = '94' . substr($waPhone, 1); }
            $waMessage = "Hello {$invoice->client->name},\n\nYour invoice *{$invoice->invoice_number}* from {$studioNameWa} is ready.\n\n*Total: Rs. " . number_format($invoice->total_amount, 2) . "*\nDue: " . $invoice->due_date->format('d M Y') . "\n\nView & download here:\n{$invoice->share_url}\n\nThank you!";
