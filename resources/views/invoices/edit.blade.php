...
<div x-data="invoiceForm()" class="grid grid-cols-3 gap-4">
    ...
    <div class="col-span-2 space-y-4">
        ...
        <div class="flex items-center justify-between mb-2">
            <div>
                <h1 class="text-2xl font-bold" :class="dark ? 'text-white' : 'text-gray-900'">Edit Invoice</h1>
                <p class="text-gray-400 text-sm mt-0.5">Invoices &gt; Edit {{ $invoice->invoice_number }}</p>
            </div>
        ...
        <div x-show="step === 3" class="rounded-xl p-5" :style="dark ? 'background:#1a1d2e;border:1px solid #252840' : 'background:#fff;border:1px solid #e5e7eb'">
            ...
            <div>
                ...
                <textarea x-model="form.terms" rows="4"
                          class="w-full text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-primary"
                          :style="dark ? 'background:#252840;color:#fff;border:1px solid #2d3154' : 'background:#f9fafb;color:#111827;border:1px solid #e5e7eb'"></textarea>
            </div>
        </div>
...
<form id="invoiceSubmitForm" method="POST" action="{{ route('invoices.update', $invoice) }}" class="hidden">
    @csrf
    @method('PUT')
...
@php
    ...
    $itemsArray = $invoice->items->map(function($item) {
    ...
    $pageDataArray = [
        ...
        'today' => $invoice->issue_date->format('Y-m-d'),
        ...
        'invoice' => [
            ...
            'project_event' => $invoice->project_event,
            'type' => $invoice->type,
            'discount_percent' => (float) $invoice->discount_percent,
            'vat_percent' => (float) $invoice->vat_percent,
...
<script>
    ...
    function invoiceForm() {
        return {
            step: 1,
            form: {
                client_id: String(pageData.invoice.client_id),
                client_name: pageData.invoice.client_name || '',
                client_address: pageData.invoice.client_address || '',
                client_city: pageData.invoice.client_city || '',
                project_event: pageData.invoice.project_event || '',
                quotation_id: null,
                invoice_number: pageData.nextNumber,
                type: pageData.invoice.type,
                issue_date: pageData.today,
                due_date: pageData.dueDate,
                discount_percent: pageData.invoice.discount_percent,
                vat_percent: pageData.invoice.vat_percent,
                paid_amount: pageData.invoice.paid_amount,
                payment_terms: pageData.invoice.payment_terms || 'Due on Receipt',
                terms: pageData.invoice.terms || '',
                status: pageData.invoice.status,
                items: pageData.invoice.items.length > 0 ? pageData.invoice.items : [
