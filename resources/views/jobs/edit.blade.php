...
<div class="max-w-3xl mx-auto">
    ...
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('jobs.show', $job) }}" class="text-gray-400 hover:text-white transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
            </svg>
        </a>
        <div>
            <h1 class="text-2xl font-bold" :class="dark ? 'text-white' : 'text-gray-900'">Edit Job</h1>
            <p class="text-gray-400 text-sm">Job Management &gt; {{ $job->job_number }}</p>
        </div>
    </div>
    ...
    <div class="rounded-xl p-6" :style="dark ? 'background:#1a1d2e;border:1px solid #252840' : 'background:#fff;border:1px solid #e5e7eb'">
        <form method="POST" action="{{ route('jobs.update', $job) }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" value="{{ $job->job_number }}" readonly
            ...
            <div class="mb-4">
                ...
                <select name="quotation_id"
                    ...
                    <option value="{{ $quotation->id }}" {{ $job->quotation_id == $quotation->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="date" name="event_date"
                           value="{{ $job->event_date ? $job->event_date->format('Y-m-d') : '' }}"
                           class="w-full text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-primary"
                           :style="dark ? 'background:#252840;color:#fff;border:1px solid #2d3154' : 'background:#f9fafb;color:#111827;border:1px solid #e5e7eb'"/>
                </div>
                <div>
                    <label class="text-gray-400 text-xs mb-1 block">Event Location</label>
                    <input type="text" name="event_location" value="{{ $job->event_location }}"
                           class="w-full text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-primary"
                           :style="dark ? 'background:#252840;color:#fff;border:1px solid #2d3154' : 'background:#f9fafb;color:#111827;border:1px solid #e5e7eb'"
            ...
            <div class="grid grid-cols-3 gap-4 mb-4">
                ...
                <div>
                    ...
                    <select name="priority" required
                        ...
                        <option value="{{ $priority }}" {{ $job->priority === $priority ? 'selected' : '' }}>
                            {{ $priority }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="budget" value="{{ $job->budget }}" min="0" step="0.01"
                           class="w-full text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-primary"
                           :style="dark ? 'background:#252840;color:#fff;border:1px solid #2d3154' : 'background:#f9fafb;color:#111827;border:1px solid #e5e7eb'"
                           placeholder="0.00"/>
                </div>
            ...
            <div class="mb-4">
                ...
                <input type="date" name="delivery_date"
                       value="{{ $job->delivery_date ? $job->delivery_date->format('Y-m-d') : '' }}"
                       class="w-full text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-primary"
                       :style="dark ? 'background:#252840;color:#fff;border:1px solid #2d3154' : 'background:#f9fafb;color:#111827;border:1px solid #e5e7eb'"/>
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="notes" rows="3"
                          class="w-full text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-primary"
                          :style="dark ? 'background:#252840;color:#fff;border:1px solid #2d3154' : 'background:#f9fafb;color:#111827;border:1px solid #e5e7eb'"
                          placeholder="Internal notes...">{{ $job->notes }}</textarea>
            </div>
