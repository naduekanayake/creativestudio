...
<div class="max-w-2xl mx-auto">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-bold" :class="dark ? 'text-white' : 'text-gray-900'">Edit Reminder</h1>
            <p class="text-gray-400 text-sm">Reminders &gt; Edit</p>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('reminders.update', $reminder) }}">
            ...
            <div class="mb-4">
                ...
                <select name="client_id"
                    ...
                    <option value="{{ $client->id }}" {{ $reminder->client_id == $client->id ? 'selected' : '' }}>
                        {{ $client->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <select name="type" required
                        ...
                        <option value="{{ $type }}" {{ $reminder->type === $type ? 'selected' : '' }}>{{ $type }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="priority" required
                        ...
                        <option value="{{ $priority }}" {{ $reminder->priority === $priority ? 'selected' : '' }}>{{ $priority }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="date" name="remind_date"
                           value="{{ $reminder->remind_date->format('Y-m-d') }}" required
                           class="w-full text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-primary"
                           :style="dark ? 'background:#252840;color:#fff;border:1px solid #2d3154' : 'background:#f9fafb;color:#111827;border:1px solid #e5e7eb'"/>
                </div>
                <div>
            ...
            <div class="mb-4">
                ...
                <select name="status" required
                    ...
                    <option value="{{ $status }}" {{ $reminder->status === $status ? 'selected' : '' }}>{{ $status }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="description" rows="3"
                          class="w-full text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-primary"
                          :style="dark ? 'background:#252840;color:#fff;border:1px solid #2d3154' : 'background:#f9fafb;color:#111827;border:1px solid #e5e7eb'"
                          placeholder="Additional notes...">{{ $reminder->description }}</textarea>
            </div>
